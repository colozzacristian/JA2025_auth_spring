...
import it.eforhum.auth_module.dtos.RecoveryAuthReqDTO;
...
public class PasswordRecoveryAuthServlet extends HttpServlet{ 
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException {
        ...
        try{    
            resp.getWriter().write(objectMapper.writeValueAsString(new TempTokenRespDTO(t.getTokenValue())));
        }catch(IOException e){
            if(logger.isLoggable(Level.SEVERE))
                logger.log(Level.SEVERE, format("Failed to write recovery JWT to response for user: %s", u.getEmail()), e);
    ...
    private User performChecks(HttpServletRequest req, HttpServletResponse resp){
        ...
        if(!tokenStore.getOtpTokens().isTokenValid(recoveryAuthDTO.email(), recoveryAuthDTO.otp())){
